...
const funcToRun = ({ inContainerEl, inDataStore, inDom, inColumns }) => {
    const htmlElement = inDom.getSearchInput(inContainerEl);

    if (htmlElement.classList.contains('allColumns')) {
        htmlElement.addEventListener("input", (e) => {
            inputFuncToRun({ e, inDataStore, inColumns, inContainerEl, inDom });
        });
    };
